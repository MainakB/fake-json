...
const fetchAvgBuildDuration = async (ci_team_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_AVG_DURATION_BY_TEAM.DEF_QUERY,
    ci_team_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchTotalJobCategoryCountByTeam = async (ci_team_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_TOTAL_JOB_CATEGORY_COUNT_BY_TEAM.DEF_QUERY,
    ci_team_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchTotalJobSubCategoryCountByTeam = async (ci_team_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_TOTAL_JOB_SUB_CATEGORY_COUNT_BY_TEAM.DEF_QUERY,
    ci_team_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchSuccesTrendsByJob = async (ci_job_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_JOBS_PASS_TREND.DEF_QUERY,
    ci_job_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchTenantsTrendsByJob = async (ci_job_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_JOBS_PASS_BY_TENANT.DEF_QUERY,
    ci_job_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchDailySuccessRateChangeByTeam = async (ci_team_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_TOTAL_DAILY_SUCCES_CHANGE_BY_TEAM.DEF_QUERY,
    ci_team_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchCurrentFailingByTeam = async (ci_team_name) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_FAILING_COUNT_BY_TEAM.DEF_QUERY,
    ci_team_name
  );
  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
...
const fetchAllJobsByTeam = async (ci_team_name, start_time, end_time) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_ALL_JOBS_BY_TEAM.DEF_QUERY,
    start_time,
    end_time,
    ci_team_name
  );
...
const fetchAllJobs = async () => {
  ...
  const query_to_run = format(PG_SELECT_QUERIES.Q_FETCH_ALL_JOBS.DEF_QUERY);

  const result_query_run = await dbService.runQuery(query_to_run);
  return result_query_run;
};

const fetchJobTrendByTenantLine = async (job_name, start_time, end_time) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_JOB_TREND_BY_TENANT.DEF_QUERY,
    job_name,
    start_time,
    end_time
  );
...
const deleteTpFailedScripts = async (jobName, root_job) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_DELETE_QUERIES.Q_DELETE_TP_SCRIPTS.DEF_QUERY,
    jobName,
    root_job
  );
  const result_query_run = await dbService.runQuery(query_to_run);
...
const insertIntoTpMaster = (data) => {
  const query_to_run = format(
    PG_INSERT_QUERIES.Q_INSERT_TP_MASTER.DEF_QUERY,
    data
  );
  return query_to_run;
};

const insertIntoTpMasterExecution = (data) => {
  const query_to_run = format(
    PG_INSERT_QUERIES.Q_INSERT_TP_MASTER_EXECUTION.DEF_QUERY,
    data
  );
...
const insertIntoTpScriptMaster = (data) => {
  const query_to_run = format(
    PG_INSERT_QUERIES.Q_INSERT_TP_SCRIPT_MASTER.DEF_QUERY,
    data
  );
...
const insertIntoTpScriptMasterExec = (data) => {
  const query_to_run = format(
    PG_INSERT_QUERIES.Q_INSERT_TP_SCRIPT_MASTER_EXEC.DEF_QUERY,
    data
  );
...
const insertIntoTpScriptFailures = (data) => {
  const query_to_run = format(
    PG_INSERT_QUERIES.Q_INSERT_TP_SCRIPT_FAILURES.DEF_QUERY,
    data
  );
...
const populateTpTables = async (querySet) => {
  const dbService = new DbService();
  const result_query_run_proc = await dbService.runTransaction(querySet);
  return result_query_run_proc;
};

const fetchByTenantDateStatus = async (start_timestamp, end_timestamp) => {
  const dbService = new DbService();

  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_BY_TENANT_DATE_STATUS.DEF_QUERY,
    start_timestamp,
    end_timestamp
  );
...
const fetchByTenantTeamDateSatatus = async (start_timestamp, end_timestamp) => {
  const dbService = new DbService();
  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_BY_TENANT_TEAM_DATE_STATUS.DEF_QUERY,
    start_timestamp,
    end_timestamp
  );
  const result_query_run = await dbService.runQuery(query_to_run);
...
const fetchByTenantTeamDateScriptStatus = async (
  start_timestamp,
  end_timestamp
) => {
  const dbService = new DbService();
  const query_to_run = format(
    PG_SELECT_QUERIES.Q_FETCH_BY_TENANT_TEAM_DATE_SCRIPT_STATUS.DEF_QUERY,
    start_timestamp,
    end_timestamp
  );
  const result_query_run = await dbService.runQuery(query_to_run);
...
module.exports = {
  ...
  fetchAllJobsByTeam,
  fetchAllJobs,
  fetchSuccesTrendsByJob,
  fetchTenantsTrendsByJob,
  fetchJobTrendByTenantLine,
  insertIntoTpMaster,
  insertIntoTpMasterExecution,
  populateTpTables,
  insertIntoTpScriptMaster,
  insertIntoTpScriptMasterExec,
  insertIntoTpScriptFailures,
  deleteTpFailedScripts,
  fetchByTenantDateStatus,
  fetchByTenantTeamDateSatatus,
  fetchByTenantTeamDateScriptStatus,
